...
mapfile -d '' -t AUDIO_FILES < <(
  find "$AUDIO_DIR" -type f \( -name "*.mp3" -o -name "*.m4a" -o -name "*.ogg" -o -name "*.wav" \) -print0 | sort -z
)

echo "Found ${#STORY_FILES[@]} story files."
echo "Found ${#PDF_FILES[@]} PDF files."
echo "Found ${#AUDIO_FILES[@]} audiobook files."
...
for path in sorted(AUDIO_DIR.rglob("*")):
    if path.is_file() and path.suffix.lower() in [".mp3", ".m4a", ".ogg", ".wav"]:
        items.append({
            "title": clean_title(path),
            "path": path.as_posix(),
            "type": "audio",
...

{
  ...
  echo "## Audiobooks (${#AUDIO_FILES[@]} total)"
  echo ""
...

{
  ...
  for FILE in "${STORY_FILES[@]}" "${PDF_FILES[@]}" "${AUDIO_FILES[@]}"; do
    REL_PATH="${FILE#./}"
    echo ''
    echo '  <url>'
